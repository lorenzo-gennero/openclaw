...
- **Multi-agent safety** (assume other agents may be working concurrently):
  - Do **not** create/apply/drop `git stash` entries unless explicitly requested (includes `git pull --rebase --autostash`). Keep unrelated WIP untouched and avoid cross-cutting state changes.
...
## Operational Reference (load on demand)

Rare workflows are documented in `.agents/reference/` — read the relevant file when needed:

- `exe-dev-ops.md` — VM access, gateway restart, config
- `ghsa-workflow.md` — Security advisory patch/publish
- `npm-publish.md` — NPM + 1Password publish/verify
- `plugin-release.md` — Plugin release fast path
- `release-notes.md` — Changelog and release notes conventions
